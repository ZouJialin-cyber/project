...
def plot_lines_and_intersections(image, valid_intersections, output_folder_path, base_name):
    """
    ...
    # 绘制连接四个交点的线段
    for i in range(len(sorted_intersections)):
        start_point = sorted_intersections[i]
        end_point = sorted_intersections[(i + 1) % len(sorted_intersections)]
        plt.plot([start_point[0], end_point[0]], [start_point[1], end_point[1]], 'g-', linewidth=1)

    plt.axis('off')
    output_file_path = os.path.join(output_folder_path, base_name + '_annotated.tif')
...
folder_path = r"D:\HE_program\HE\HE_data\chip\he_data_stage_1\out\out_label_json"
# 指定包含对应的tif图像的文件夹路径
tif_folder_path = r"D:\HE_program\HE\HE_data\chip\he_data_stage_1\out\out_img"
# 指定保存新图的目的文件夹路径
output_folder_path = r"D:\HE_program\HE\HE_data\chip\he_data_stage_1\out\test2"
